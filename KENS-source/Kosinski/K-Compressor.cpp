...
write:
	if (ICount==1)
	{
		...
		if (++BFP==16) { fwrite(&BITFIELD,2,1,Dst); fwrite(&Data,DS,1,Dst); BITFIELD=BFP=DS=0; }	
...
//----------------------------------------------------------------------------------------------------------------
	...
	fwrite(&BITFIELD,2,1,Dst); fwrite(&Data,DS,1,Dst); BITFIELD=BFP=DS=0;

	if (Moduled)
	{
		fseek(Dst, 16-(ftell(Dst)-2)%16, SEEK_CUR);
		CompBytes+=BSize;
		Buffer+=BSize;
	...
	if (ftell(Dst) & 1) fseek(Dst, 1, SEEK_CUR);
	fclose(Dst);
