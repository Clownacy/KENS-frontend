...
BOOL APIENTRY DllMain(HANDLE, DWORD ul_reason_for_call, LPVOID)
{
    ...
    switch (ul_reason_for_call)
	{
		case DLL_PROCESS_ATTACH:
			...
			GetTempFileName(TempPath, "eni", 0, LocalSrc);
			GetTempFileName(TempPath, "eni", 0, LocalDst);
			return TRUE;
...
long __cdecl Comp(char *SrcFile, char *DstFile, bool Padding)
{
	...
	Result = EComp(LocalSrc, LocalDst, Padding);
...
{
	if (!SameSource(SrcFile)) CopyFile(SrcFile, LocalSrc, FALSE);
	Result = EDecomp(LocalSrc, LocalDst, Pointer, Padding);
	CopyFile(LocalDst, DstFile, FALSE);
	return Result;
}

//-- Compression To Buffer ----------------------------------------------------------------------
long __cdecl CompToBuf(char *SrcFile, char *&DstBuffer, long *BufSize, bool Padding)
{
	if (!SameSource(SrcFile)) CopyFile(SrcFile, LocalSrc, FALSE);
	Result = EComp(LocalSrc, LocalDst, Padding);
...
long __cdecl DecompToBuf(char *SrcFile, char *&DstBuffer, long *BufSize, long Pointer, bool Padding)
{
	if (!SameSource(SrcFile)) CopyFile(SrcFile, LocalSrc, FALSE);
	Result = EDecomp(LocalSrc, LocalDst, Pointer, Padding);
	*BufSize = LoadBuffer(DstBuffer);
...
long __stdcall VBComp(VARIANT _SrcFile, VARIANT _DstFile, bool Padding)
{
...
long __stdcall VBDecomp(VARIANT _SrcFile, VARIANT _DstFile, long Pointer, bool Padding)
{
	char SrcFile[MAX_PATH]; OLECHARsToChars(_SrcFile.bstrVal, SrcFile);
	char DstFile[MAX_PATH]; OLECHARsToChars(_DstFile.bstrVal, DstFile);
	if (!SameSource(SrcFile)) CopyFile(SrcFile, LocalSrc, FALSE);
...
double __cdecl GMComp(char *SrcFile, char *DstFile, double Padding)
{
	if (!SameSource(SrcFile)) CopyFile(SrcFile, LocalSrc, FALSE);
	Result = EComp(LocalSrc, LocalDst, (bool)Padding);
	CopyFile(LocalDst, DstFile, FALSE);
	return Result;
}

//-- Game Maker Decompression -------------------------------------------------------------------
double __cdecl GMDecomp(char *SrcFile, char *DstFile, double Pointer, double Padding)
{
	if (!SameSource(SrcFile)) CopyFile(SrcFile, LocalSrc, FALSE);
	Result = EDecomp(LocalSrc, LocalDst, (long)Pointer, (bool)Padding);
	CopyFile(LocalDst, DstFile, FALSE);
	return Result;
}
