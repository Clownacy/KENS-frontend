...
unsigned char packet_length = 0x0B;
unsigned char bitmask = 0x00;
unsigned short common_value = 0x0000;
unsigned short tempval = 0x0000;
short incrementing_value = 0x0000;
...
//						- is stored in the full 80x80 block version
	...
	for(loopcount = 0; loopcount < infilelength; loopcount += 2)
	{
		tempval2 |= (*(pointer + loopcount) << 8);
		tempval2 |= *(pointer + loopcount + 1);
	}

	bitmask = (tempval2 >> 0x0B);
	for(loopcount = 0; loopcount < 0x0A; loopcount++)
	{
		if(((tempval2 >> (0x0A - loopcount)) & 0x0001) == 0)
		{
			packet_length--;
		}
		else
...
//	printf("Calculating common and increment values......");
	calc_headerval();
//	printf("\tDone!\n");
	unsigned char crapway1 = (incrementing_value >> 8);
...
//	printf("Compressing file......");
	for(;!done;)
	{
		...
		for(commonp = 0; (checkval(commonp * 2) == common_value) && (commonp < 0x10); commonp++){}
		for(incp = 0; (checkval(incp * 2) == (incrementing_value + incp)) && (incp < 0x10); incp++){}
		...
		for(comprep = 0; (comprep < 0x0F) && (checkval(comprep * 2) != common_value) && (checkval(comprep * 2) != incrementing_value) && (checkval(comprep * 2) != checkval(comprep * 2 + 2)) && (checkval(comprep * 2) != (checkval(comprep * 2 + 2) + 1)) && (checkval(comprep * 2) != (checkval(comprep * 2 + 2) - 1)); comprep++){}

		if(incp > 0)
		{
			writeinc(incp);
			incrementing_value += incp;
			continue;
		}
		if(commonp > 0)
		{
...
short checkval(int offsetloc)
{
	checkbuffer = (*(pointer + offset + offsetloc) << 8);
	checkbuffer |= *(pointer + offset + offsetloc + 1);
	return checkbuffer;
}
...
int writecomp(int vcomp)
{
	...
	{
		if(((bitmask << loopcount) & 0x80) != 0)
		{
			writebit((checkval(0) >> (0x0F - loopcount)) & 0x0001);
		}
	}
	for(loopcount = 0; loopcount < packet_length; loopcount++)
	{
		writebit((checkval(0) >> ((packet_length - 1) - loopcount)) & 0x0001);
	}
...
bool calc_headerval(void)
{
	...
	for(loopcount = 0; loopcount < infilelength; loopcount += 2)
	{
		commontemp = (*(pointer + commonoffset++) << 8);
		commontemp |= *(pointer + commonoffset++);
	...
	for(loopcount = lowestval; loopcount < 0x10000; loopcount++)
	{
		commonoffset = 0;
		inchitcount = 0;
		comhitcount = 0;
		...
		for(;commonoffset < infilelength;)
		{
			...
			{
				comhighestcount++;
				common_value = (unsigned short)loopcount;
			}
			if(inchitcount > inchighestcount)
			{
				inchighestcount++;
				incrementing_value = (tempincval - inchitcount);
			}
		}
	}
	return true;
